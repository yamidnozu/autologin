document.addEventListener('DOMContentLoaded', () => {
  ...
  const copyMessage = document.getElementById('copy-message'); // Nuevo elemento para mensajes
    ...
    chrome.storage.local.get(['--extension--users'], (result) => {
      ...
      users.forEach((user, index) => {
        ...
        editBtn.addEventListener('click', (event) => {
          event.stopPropagation(); // Evita que el clic también se registre en la celda
          editUser(index);
        });
  ...
  // Función para mostrar mensajes de copia
  function showCopyMessage(text) {
    copyMessage.textContent = `«${text}» copiado al portapapeles.`;
  ...
  // Manejar clics en las celdas de la tabla para copiar texto y resaltar la fila
  usersTableBody.addEventListener('click', (event) => {
    const target = event.target;

    // Evitar acciones si se hace clic en los botones de editar/eliminar
